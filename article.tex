...
$$ c \leq a $$
$$ c \leq b $$
In zato tudi manjši od infinuma teh dveh elementov. $ c \leq a \land b$.\\
...
\end{definition}\\
\begin{example}
Najbolj enostaven Boolovega kolobarja je kar kolobar $\mathbb{Z}_{2}$ z operacijo navadnega seštevanja in množenja. Še kakšen drug primer Boolovega kolobarja pa najlaže dobimo tako, da skupaj vzamemo več kopij $\mathbb{Z}_{2}$. Še več, da se pokazati, da so prav vsi Boolovi kolobarji izomorfni nekemu številu kopij $\mathbb{Z}_{2}$. Boolov kolobar moči $2^3$ je prikazan na sliki (\ref{im:boolean_ring_z_3}).
\end{example}
...
\end{enumerate}
S tem smo pokazali, da je boolov kolobar z operacijama supremum in infinum kot smo ju definirali, zgoraj mreža.

Za boolove kolobarje pa velja še celo več. Velja namreč, da je ta mreža distributivna. To najlaže pokažemo kar z direktnim računom.
$$ a \land (b \lor c) = a \land (b + c + bc) = ab + ac + abc $$
$$ (a \land b) \lor (a \land c) = ab \lor ac = ab + ac + abc $$
V boolovem kolobarju ničelni element sovpada z minimalnim elementom mreže. Prav tako tudi element $1$.
$$ a \land 0 = a  0 = 0 $$ $$ a \lor 0 = a + 0 + a0 = a $$ 
$$ a \land 1 = 1a = a $$ $$ a \lor 1 = a + 1 + a = 1 $$

\subsection{Lastnosti in primeri boolovih kolobarjev}

Ugotovili smo že da je poljubno kopij $\mathbb{Z}_2$ Boolov kolobar, če pa si pobližje pogledamo sliko (\ref{im:boolean_ring_z_3}) in sliko (\ref{im:divisors_of_30_hasse_diagram}) vidimo da sta si izomorfni, zanima nas ali bi lahko to kako posplošili.

\begin{statement}
Boolov kolobar moči $2^n$ je izomorfen mreži vseh deliteljev naravnega števila $m$, ki ima $n$ različnih praštevilskih deliteljev, ne deli pa ga kvadrat kakega praštevila.
\end{statement}

\begin{proof}
(Zgolj skica) naj bo $m = p_1p_2 \dots p_n$, potem je vsak delitelj števila $m$ oblike $k = p_1^{i_1}p_2^{i_2} \dots p_n^{i_n}$, kjer $i \in \{0,1\}$. Izomorfizem bo potem kar $\varphi(p_1^{i_1}p_2^{i_2} \dots p_n^{i_n}) = (i_1, i_2, \dots, i_n)$, ki je očitno bijektiven, to da pa ohranja red v obeh smereh pa preprosto preverimo z računom.
\end{proof}
