...
Mreža je množica z dodatno strukturo (delno urejenostjo), ki zadošča pogoju, da ima poljuben par elementov infimum in supremum. Najprej si bomo pogledali mreže s stališča matematične logike in urejenosti, kasneje pa si jih bomo ogledali še s stališča algebraičnih operacij nad njimi, ter pokazali, zakaj sta ta dva pogleda ekvivalentna.
...
\end{itemize}
Govorimo $a$ je manjši ali enak $b$, občasno tudi $a$ je pod $b$.
\end{definition}

\begin{remark}
Za lažje izražanje definiramo še drugo relacijo $a \geq b \iff b \leq a$, ki je očitno tudi delna urejenost.
\end{remark}
\\
\\
\begin{remark}
...
\end{statement}
\begin{proof}
Preverili bomo da ta relacija zadošča vsem zahtevam. Spomnimo se, da $a$ deli $b$ natanko tedaj, kadar obstaja tako celo število $k$, da zadosti enakosti $b = ka$, oziroma 
$$ a \mid b \iff \exists k \in \mathbb{Z}. \ b = ka.$$
\begin{itemize}
\item Refleksivnost: $a = 1*a$, torej $a \mid a$.
\item Antisimetričnost:
$$a \mid b \implies b = k_1a, \quad b \mid a \implies a = k_2b$$
 vstavimo $a$ v prvo enakost in dobimo $b = k_1k_2 b$, torej $k_1 = k_2^{-1}$, torej $k_1 = k_2 = 1$ in dobimo $b = a$.
...
\end{example}
\\
Za lažjo predstavo mrež si lahko pomagamo s Hassejevimi diagrami. V primeru, ko je naša množica linearno urejena, nas Hassejev diagram spominja na premico in od tod sledi tudi izraz linearna urejenost.
\\

\begin{definition}
Naj bo $\mathcal{L}$ delno urejena množica glede na neko relacijo, ki jo označimo z $\leq$. Hassejev diagram je graf, katerega točke so elementi $\mathcal{L}$, med točkama $x$ in $y$ pa je povezava natanko tedaj, kadar velja: $$ x \leq y \land \nexists z \in \mathcal{L}. \ x \leq z \leq y$$
\end{definition}
...
\begin{definition}
Element $S$ je \textbf{supremum} elementov $x$ in $y$, če velja: 
\begin{itemize}
...
\end{itemize}
Torej je $S$ natančna zgornja meja $x$ in $y$, če je njuna zgornja meja, hkrati pa je vsaka od $S$ različna zgornja meja večja ali enaka $S$. Označimo: $S = x \lor y$. 
\end{definition}


\begin{definition}
Element $s$ je \textbf{infimum} elementov $x$ in $y$, če velja: 
\begin{itemize}
...
\label{sec:osnovni_primeri}

Od prej vemo, da je poljubna podmonožica $\mathbb{R}$, urejena glede na relacijo $\leq$ mreža.
Za poljubni števili $x,y$ velja: $x \lor y = max\{x,y\}$ in $x \land y = min\{x,y\}$, seveda pa sta tako infimum kot supremum v mreži. Opazimo, da so te množice vedno linearno urejene.
\\\\
Množica naravnih števil pa nam poleg urejenosti z $\leq$ omogoča tudi ureditev z $\mid$. V tem primeru dobimo Hassejev diagram iz slike (\ref{im:natural_nubmers_divisibility_hasse_example}). Ta množica nima linearne ureditve. Opazimo pa, da elementa $0$ in $1$ na nek način zaključujeta mrežo, saj so vsa naravna števila pod $0$ (delijo $0$), hkrati pa $1$ deli vsa ostala naravna števila. S slike tudi lepo opazimo, da so v prvem nivoju urejenosti samo praštevila, v drugem nivoju urejenosti števila z natančno dvema deliteljema in v $n$-tem nivoju praštevila z $n$ delitelji.
\\\\
\begin{remark}
...
Pomembne mreže pa predstavljajo tudi mreže deliteljev posameznega naravnega števila. To lepo vidimo na sliki (\ref{im:divisors_of_30_hasse_diagram}). Delno urejene množice si lahko predstavimo tudi zgolj s Hassejevim diagramom. Poglejmo si nekaj množic, ki niso mreže in razložimo zakaj ne. Če si pogledamo sliko (\ref{im:not_lattice_example}) lahko hitro vidimo, zakaj te množici nista mreži. Množica na sliki (\ref{im:not_lattice_1}) ni mreža, saj elementa $c$ in $f$ nimata natančno določenega supremuma. Vidimo, da bi to lahko bila tako element $e$ kot tudi element $b$, vendar pa ju med samo ne moremo primerjati. Množica na sliki (\ref{im:not_lattice_1}) zato ni mreža. Bi pa postala mreža, če bi odstranili povezamo med $c$ in $e$. Če pa pogledamo sliko (\ref{im:not_lattice_2}) vidimo, da je problematičen element $f$. Tako recimo $c \lor f = a$, medtem, ko je $c \land f$ nedoločen in zato množica ni mreža, bi pa to postala, če bi iz nje odstranili element $f$ in tako dobili mrežo vseh deliteljev števila $12$. 
...
Pomemben primer mreže je tudi potenčna množica poljubne množice urejena glede na relacijo inkluzije ($\subseteq$), to lepo vidimo na sliki (\ref{im:hasse_diagram_natural_subset}). Da se prepričamo, da je ta množica mreža, preprosto preverimo supremume in infimume. Vidimo, da velja $x \lor y = x \cup y$ in $x \land y = x \cap y$, kar nam pojasni, zakaj se občasno za označevanje supremuma in infimuma uporablja oznaki $\cap, \cup$, saj naravno sledita iz potenčne množice kot mreže. Takoj opazimo, da je tako kot mreža $(\mathbb{N}, \mid)$ tudi ta mreža omejena, saj je $\{\}$ pod vsemi ostalimi, $\mathbb{N}$ pa nad vsemi. Prav tako pa vidimo da v $n$ tem nivoju nastopajo vse možne $n$-elementne podmnožice začetne množice.
...
Naj bo $a = (x \lor y) \lor z$ in $b = x \lor (y \lor z)$. Iz definicje ali sledi $a \geq z$ in $a \geq (x \lor y)$ in torej $a \geq x, a \geq y$. Potem velja tudi $a \geq (y \lor z)$ in $a \geq x \lor (y \lor z)$ torej $a \geq b$. 
Podobno postopamo za nasprotno stran in tako dobimo $b \geq a$. Od tod pa zaradi antisimetričnosti relacije velja $a=b$.\\
Na analogen način preverimo tudi za $\land$, le da $a$ in $b$ omejimo navzgor.
\item Absorbcija: Preverimo za $x \land (x \lor y) = x$. Iz definicije $\lor$ vemo $x \leq (x \lor y)$ in $x \leq x$, torej $x \leq x \land (x \lor y)$. Iz definicije $\land$ pa sledi $x \geq x \land (x \lor y)$.
...
\\
\begin{theorem}
Če imamo množico $\mathcal{L}$, za katero sta definirani operaciji $\lor, \land$ in če za ti dve operacji veljajo zgornji zakoni (idempotentnost, komutativost, asociativnost, absorbcija), potem je ta množica mreža.
\end{theorem}
...
Da pa bo množica skupaj s to operacijo mreža, moramo preveriti še obstoj supremuma in infimuma za poljubna dva elementa. \\
...
Analogno dokažemo, da je infimum $x$ in $y$ element $x \land y$. 
...
\newpage
\begin{definition}
Neprazna množica $\mathcal{M} \subseteq \mathcal{L}$ je \textbf{podmreža}, če je tudi sama mreža za isti operaciji in se na njih ujema. Povedano drugače $$\forall x,y \in \mathcal{M}. \ x \lor y \in \mathcal{M}, \  x \land y \in \mathcal{M}.$$
\end{definition}
...
\end{example_case}
\begin{example_case}
Sliki (\ref{im:divisors_of_30_with_sublattices} in \ref{im:lattice_1234_with_sublattices}) prikazujeti dva primera podmrež na že znanih mrežah. Tako sta na primer množica deliteljev števila $10$, kot tudi množica$\{2,6,10,30\}$ podmreža mreže deliteljev števila $30$ (slika \ref{im:divisors_of_30_with_sublattices}). Prav tako pa je $P(\{1,2,4\})$ podmreža mreže $P(\{1,2,3,4\})$ (slika \ref{im:lattice_1234_with_sublattices}).
\end{example_case}
\end{example}
...
Naravno nas zanima, ali lahko iz že obstoječih mrež na preprost način dobimo podmreže, če si pogledamo prejšnje primere lahko iz vseh razen modrega na sliki (\ref{im:divisors_of_30_with_sublattices}) ugotovimo da so to podmreže sestavljene iz elementov, ki so med dvema elementoma v mreži. Zanima nas, ali lahko to posplošimo.
\\
...
\begin{corollary}
Če v mreži $\mathcal{L}$ obstajata $0$ in $1$, potem za poljuben $x \in \mathcal{L}$ veljajo naslednje trditve:
\begin{itemize}
\item $x \lor 1 = 1$
\item $x \lor 0 = x$
\item $x \land 0 = 0$
...
\label{sec:homomorfizmi_mrez}
%Sliki mrež, ki sta si izomorfni
...
\hfill
  ...
  \draw (zero) -- (a) -- (one)  -- (d) -- (zero);
  \end{tikzpicture}
    \caption{Mreža deliteljev števila $6$}
    \label{im:lattice_1234_with_sublattices}
  \end{subfigure}
  \caption{Izomorfni mreži}
  \label{im:isomorph_lattices}
\end{figure}

Če si pogledamo zgornji mreži na sliki (\ref{im:isomorph_lattices}) opazimo, da se zdita zelo podobni. Tako kot pri vseh algebrskih strukturah tudi tukaj definiramo pojem homomorfizma, s pomočjo katerega bomo lahko podobne objekte med seboj "enačili".
...
\begin{theorem}
Bijektivna preslikava iz $L_{1}$ v $L_{2}$ je izomorfizem, če ohranja urejenost iz $L_{1}$ v $L_{2}$ in nazaj.
\end{theorem}
\begin{proof}
Naj bosta $a, b \in L_{1}$. Ker je $a \land b \leq a$ in je tudi $a \land b \leq b$, potem zaradi ohranjanja smeri velja:
$$f(a \land b) \leq f(a), f(a \land b) \leq f(b) $$
Ker je $f(a \land b)$ manjši od obeh $f(a)$ in $f(b)$ je manjši tudi od njunega infinuma.
$$ f(a \land b) \leq f(a) \land f(b) $$
S tem smo pokazali, da je $f(a \land b)$ spodnja meja elementov $f(a)$ in $f(b).$ Pokažimo, da je to še natačna spodnja meja. \\
Denimo, da $f(c) \leq f(a)\land f(b)$. To pomeni, da je 
$$ f(c) \leq f(a)$$ $$ f(c) \leq f(b) $$
Ker preslikava ohranja red v obe smeri velja:
$$ c \leq a $$
$$ c \leq b $$
zato pa je $c$ tudi manjši od infinuma teh dveh elementov. $ c \leq a \land b$.\\
Uporabimo sedaj preslikavo in ohranjanje smeri, ter dobimo:
$$ f(c) \leq f(a \land b) $$
Pokazali smo, da je vsaka druga spodnja meja elementov $f(a)$ in $f(b)$ manjša od  $f(a \land b)$. Po definiciji supremuma sedaj sledi 
$$ f(a \land b) = f(a) \land f(b) $$
Enakost 
$$ f(a \lor b) = f(a) \lor f(b) $$ 
dokažemo na analogen način. Pokazali smo, da je bijektivna preslikava, ki ohranja smer v obe smeri, homomorfizem mrež. Ker pa je naša preslikava bijektivna sledi, da je to izomorfizem mrež.
\end{proof} 
...
\begin{definition}
Mreža $\mathcal{L}$ je modulska, če za vsak $a, b, c \in \mathcal{L}$, pri pogoju, da je burek $a \leq c$, velja:
...
Da pa pokažemo da obratno ni res, pa si poglejmo mrežo na sliki (\ref{im:modular_non_distributive_lattice})\\
%Slika neke male modulske mreže, ki ni distributivna.
  ...
  \draw (zero) -- (two) -- (one) -- (three) -- (zero) -- (five) -- (one); 
  
  \end{tikzpicture}
  \caption{Modulska mreža, ki ni distributivna}
  \label{im:modular_non_distributive_lattice}
  \end{figure}


Preden se lotimo naslednjega primera, si poglejmo še formulo, ki velja za prav vsako mrežo. Naj bodo $a, b, c \in L$ in naj velja $a \leq c$. Ker je $a \leq c$ in $a \leq (a \lor b)$ sledi, da je $$a \leq (a \lor b) \land c$$Vemo tudi, da velja, da je $b \land c \leq b \leq a \lor b.$ in $b \land c \leq c$. Torej je tudi 
$$ b \land c \leq (a \lor b) \land c $$
Iz te in zgornje ugotovitve pa vidimo, da velja še
$$ a \lor (b \land c) \leq (a \lor b) \land c $$

Za lep primer modulskih mrež pa bomo pokazali, da je množica podgrup edink grupe $G$, urejena glede na vsebovanost, modulska mreža. Naprej preverimo, da je to sploh mreža. Množica gotovo ni prazna, saj vemo, da sta $\{ 1 \}$ in $G$ podgrupi edinki. Naj bosta $H$ in $K$ poljubni podgrupi $G$. Za infinum dveh podrup edink vemo, da je $H \cap K$ najmanjša podgrupa, ki ju vsebuje. Za supremum dveh podgrup pa $H \cup K$ seveda ni nujno podgrupa. Vemo pa, da je najmanjša podgrupa, ki  $H$ in $K$ vsebuje, podgrupa:
...
\section{Booleovi kolobarji}
\label{sec:boolovi_kolobarji}

\begin{definition}
Booleov kolobar $K$ je kolobar, v katerem je vsak element idempotenten. Torej $$\forall a \in K. \ a^2 =a$$
\end{definition}\\
\begin{example}
Najbolj enostaven Booleovega kolobarja je kar kolobar $\mathbb{Z}_{2}$ z operacijo navadnega seštevanja in množenja. Še kakšen drug primer Booleovega kolobarja pa najlaže dobimo tako, da skupaj vzamemo več kopij $\mathbb{Z}_{2}$. Še več, da se pokazati, da so prav vsi Booleovi kolobarji izomorfni nekemu številu kopij $\mathbb{Z}_{2}$. Booleov kolobar moči $2^3$ je prikazan na sliki (\ref{im:boolean_ring_z_3}).
\end{example}
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
    \caption{Mreža na kolobarju $\mathbb{Z}_2 \oplus \mathbb{Z}_2 \oplus \mathbb{Z}_2$}
    \label{im:boolean_ring_z_3}
  \end{figure} 
...
\end{proof}
Pokažimo sedaj, da lahko iz Booleovega kolobarja pridemo do mreže. V ta namen definirajmo operaciji supremum in infinum in pokažimo, da zadoščata aksimom mreže (idempotentnost, komutaticnost, asociativnost, absorbcija).
$$ a \lor b = a + b + ab $$
$$ a \land b = ab $$
\begin{enumerate}
...
\item(Absorbcija)
$$ a \land (a \lor b) = a \land (a + b + ab) = a^2 + ab + ab = a$$
$$ a \lor (a \land b) = a \lor ab = a + ab + a^2b = a $$
\end{enumerate}
S tem smo pokazali, da je Booleov kolobar z operacijama supremum in infinum kot smo ju definirali, zgoraj mreža.
...
$$ a \land (b \lor c) = a \land (b + c + bc) = ab + ac + abc $$
$$ (a \land b) \lor (a \land c) = ab \lor ac = ab + ac + abc $$
V Booleovem kolobarju ničelni element sovpada z minimalnim elementom mreže. Prav tako tudi element $1$.
$$ a \land 0 = a  0 = 0 $$ $$ a \lor 0 = a + 0 + a0 = a $$ 
$$ a \land 1 = 1a = a $$ $$ a \lor 1 = a + 1 + a = 1 $$

\subsection{Lastnosti in primeri Booleovih kolobarjev}

Ugotovili smo že da je poljubno kopij $\mathbb{Z}_2$ Booleov kolobar, če pa si pobližje pogledamo sliko (\ref{im:boolean_ring_z_3}) in sliko (\ref{im:divisors_of_30_hasse_diagram}) vidimo da sta si izomorfni. Zanima nas, ali bi lahko to kako posplošili.

\begin{statement}
Booleov kolobar moči $2^n$ je izomorfen mreži vseh deliteljev naravnega števila $m$, ki ima $n$ različnih praštevilskih deliteljev, ne deli pa ga kvadrat kakega praštevila.
\end{statement}

\begin{proof}
(Zgolj skica) naj bo $m = p_1p_2 \dots p_n$, potem je vsak delitelj števila $m$ oblike $k = p_1^{i_1}p_2^{i_2} \dots p_n^{i_n}$, kjer $i \in \{0,1\}$. Pripdajoč izomorfizem bo potem kar $\varphi(p_1^{i_1}p_2^{i_2} \dots p_n^{i_n} ) = (i_1, i_2, \dots, i_n)$, ki je očitno bijektiven, to da pa ohranja red v obeh smereh pa preprosto preverimo z računom.
\end{proof}
